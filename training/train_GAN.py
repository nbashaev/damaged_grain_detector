...
model = GAN()
model.hybridize()
#model.load_parameters(os.path.join(ARCHIVE_FOLDER, 'unet-GAN-14-0.629.params'), ctx=ctx)
...
generator_trainer = mx.gluon.Trainer(model.generator.collect_params(), 'sgd',
									{'learning_rate': 0.0001, 'momentum': 0.9, 'lr_scheduler': scheduler})
...
'''.format(model(samples['train'][0].expand_dims(axis=0))[1].sum().asscalar()))
...
for e in tbar:
	loss_recorder.add_val('learning_rate', discriminator_trainer.learning_rate)
	...
	for ind, (x, y) in enumerate(train_data):
		with autograd.record():
			...
			if np.random.rand() < 0.4:
				...
				error = 0.9 * segm_loss + 0.1 * adv_loss
